/*******************************************************************************
  Copyright(c) 2012 Joaquin Bogado.
...
//gcc -DQHY5T_TEST -Wall -o qhy5t qhy5tdriver.c -lusb -lcfitsio -lpthread
...
int qhy5t_stopcapture(pthread_t t){
...
void *  qhy5t_start_exposure(void * ptr){
...
void * qhy5t_read_exposure(qhy5t_driver *qhy5t){
	/*Vamos a ver si puedo implementar doble buffering*/
	...
	//memcpy(buffer[i],qhy5t->image,qhy5t->width * qhy5t->height);
	return actual;
}
...
/*
 * Function call for write fits
 * Write by G.Spezzano for QHY6CAPTURE
 * */
...
int write_fits(qhy5t_driver *qhy5t, int width, int height, int gain, int exp, char *fn ){
	/*
	 * Original function 
	 * int WriteFitFile(char *fname, unsigned short *array, int width, int height  )
	 * Write by G.Spezzano for QHY6CAPTURE
	 * */
	char * fname = fn;
	unsigned short * array = qhy5t->image;
	fitsfile *fptr;       /* pointer to the FITS file, defined in fitsio.h */
	int status;
	long  fpixel, nelements, exposure=exp;
	...
	if ( fits_update_key(fptr, TSTRING, "SOFTWARE", "QHY5TCAPTURE","", &status) ) printerror( status );
	if ( fits_update_key(fptr, TSTRING, "CAMERA", "QHY5T","", &status) ) printerror( status );
	if ( fits_update_key(fptr, TLONG,   "GAIN", &(qhy5t->gg1),"", &status) )     printerror( status );
	//if ( fits_update_key(fptr, TLONG,   "OFFSET", &offset,"", &status) ) printerror( status );
	//if ( fits_update_key(fptr, TLONG,   "AMP", &amp,"", &status) ) printerror( status );
	if ( fits_update_key(fptr, TLONG,   "EXPOSURE", &exposure, "Total Exposure Time", &status) ) printerror( status );
...
int main (int argc,char **argv){
	...
	//error = qhy5t_start_exposure(qhy5t); //THREADED FUNCTION!!
	thr_error = pthread_create( &exposure_thread, NULL, qhy5t_start_exposure, (void*) qhy5t);
	//pthread_join(exposure_thread, NULL); //NO JOIN... read_image must ask if is exposing a new frame
	//usleep(10000);
	if (thr_error){printf("Error %d\n", error); exit(error);}
	
	if (count == 0){
		//for QHYImager compatibility ... 
		//the name for the image will be basename 
		write_fits(qhy5t, width, height, gain, etime, basename);
	}
	else{
		for(i = 0; i < count; i++) {
			...
			//if (error){printf("Error %d\n", error); exit(error);}
			if (!strncmp(fmt,"ppm", 10))
				write_ppm(data, qhy5t->width, qhy5t->height, imagename);
			else{
				write_fits(qhy5t, width, height, gain, etime, imagename);
			}
		}
	}
	qhy5t_stopcapture(exposure_thread);
	qhy5t_close(qhy5t);
	return 0;
}
